...
check-world:
	...
	@$(BIN)/python -c "import json,glob; m=json.load(open(sorted(glob.glob('kit/world/*/manifest.json'))[-1])); c=m.get('counts',{}); print('world', m.get('world_id'), '-', c.get('__total__', c.get('total', sum(c.values()))), 'pages')"
